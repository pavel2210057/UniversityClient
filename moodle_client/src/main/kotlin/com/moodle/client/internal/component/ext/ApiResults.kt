...
internal fun<F : Transformable<T>, T> ApiResult<F>.transformOrFailure(): ApiResult<T> = when(this) {
    is ApiResult.Success -> ApiResult.success(data.transform())
    is ApiResult.Failure -> ApiResult.failure(cause)
}

internal fun<F : List<Transformable<T>>, T> ApiResult<F>.transformListOrFailure(): ApiResult<List<T>> = when(this) {
